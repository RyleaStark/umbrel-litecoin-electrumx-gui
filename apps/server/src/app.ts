...
export interface ElectrumXGuiService {
...
export function buildApp({ service, serveUi = true }: { service: ElectrumXGuiService; serveUi?: boolean }): FastifyInstance {
  const app = Fastify({
    logger: false,
    bodyLimit: 16 * 1024,
  });
  ...
  app.get("/api/status", async (_request, reply) => {
    try {
      ...
      return reply.code(503).send({ error: "ElectrumX status is temporarily unavailable" });
    }
  });
  app.get("/api/connections", async () => service.getConnections());

  app.get("/v1/electrumx/version", async (_request, reply) => {
    try {
  ...
  app.get("/v1/electrumx/syncPercent", async () => {
    try {
  ...
  app.get("/v1/electrumx/electrum-connection-details", async () => {
