...
describe("GUI API client", () => {
  it("validates status responses", async () => {
    const fetchFn = vi.fn(async () => new Response(JSON.stringify({ state: "ready", version: "2.0.0", coreHeight: 110, indexedHeight: 110, percent: 100, message: "ElectrumX is synchronized" }), { status: 200 }));
  ...
  it("rejects malformed status responses", async () => {
    ...
    await expect(fetchStatus(fetchFn)).rejects.toThrow("ElectrumX returned an invalid status response");
  });
  ...
  it("validates connection responses", async () => {
    const payload = {
      ...
      tor: { address: "example.onion", port: 50001, connectionString: "example.onion:50001", transport: "tcp" }
    };
