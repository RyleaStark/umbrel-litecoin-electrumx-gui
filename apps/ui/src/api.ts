...
async function request(path: string, fetchFn: typeof fetch): Promise<unknown> {
  ...
  try {
    ...
    throw new Error("ElectrumX is temporarily unavailable");
  }
  if (!response.ok) throw new Error("ElectrumX is temporarily unavailable");
  try {
    return await response.json();
  } catch {
    throw new Error("ElectrumX returned an invalid response");
  }
}
...
export async function fetchStatus(fetchFn: typeof fetch = fetch): Promise<IndexerStatus> {
  ...
  if (!result.success) throw new Error("ElectrumX returned an invalid status response");
  return result.data;
}
...
export async function fetchConnections(fetchFn: typeof fetch = fetch): Promise<ConnectionDetails> {
  ...
  if (!result.success) throw new Error("ElectrumX returned an invalid connection response");
  return result.data;
}
