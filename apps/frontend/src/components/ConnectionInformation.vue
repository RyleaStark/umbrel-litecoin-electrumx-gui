    ...
    <div class="flex flex-col md:grid md:grid-cols-12 md:gap-8">
      ...
      >
        <div class="max-w-fit p-4 bg-white rounded-lg shadow">
          <!-- QR Code -->
          <qr-code
            :value="connectionInfo && connectionInfo.connectionString"
            :size="220"
...
export default {
  ...
  computed: {
    ...mapState({
      electrumx: function(state) {
        return state.electrumx;
      },
      connectionInfo: function(state) {
        if (this.selectedNetwork === "local") {
          return state.electrumx.connectionInfo.local;
        } else {
          return state.electrumx.connectionInfo.tor;
        }
      },
    }),
  },
