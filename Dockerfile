...
USER 1000:1000
EXPOSE 3007
HEALTHCHECK --interval=30s --timeout=3s --start-period=10s --retries=3 CMD ["/nodejs/bin/node", "-e", "fetch('http://127.0.0.1:3007/ping').then(r=>{if(!r.ok)process.exit(1)}).catch(()=>process.exit(1))"]
